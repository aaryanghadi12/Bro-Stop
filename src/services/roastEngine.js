// Sanjay Dutt (Sanju Baba / Munna Bhai) Style Roast Engine with Gemini API integration

const ROAST_TEMPLATES = {
  food_delivery: {
    SAVAGE: [
      (desc, amt, freq) => `Aey Bhai! ${freq > 1 ? `Do din mein ${freq} baar Swiggy?` : 'Bole toh ek aur Swiggy order?'} ₹${amt} uda diye! Fridge showpiece ke liye rakha hai kya mamu?`,
...
/**
 * Generates local intelligent Hinglish roast in Sanjay Dutt / Munna Bhai persona
 */
export function generateLocalRoast({ description, amount, category, tier, recentCount = 1 }) {
  if (tier === 'SAFE') {
  ...
  const roastFn = list[randomIndex];

  return roastFn(description || 'Faltu kharcha', amount, recentCount);
}

/**
 * Generates AI Roast using Gemini API in Sanjay Dutt / Munna Bhai persona
 */
...
Return ONLY the roast text, nothing else.`;
  ...
  try {
    ...
    if (!response.ok) {
      console.warn('Gemini API call returned non-200, falling back to local roast engine');
      return generateLocalRoast({ description, amount, category, tier });
    }
...
/**
 * Calculates what this impulse money would become if invested in SIP (12% CAGR)
 */
